// app.js — Phase 4.8 Theme Restoration Fix
...
function parseNarrativeMarkdown(text) {
    if (!text) return "";
    // Bold Headers for themes
    let html = text.replace(/\*\*(.*?)\*\*/g, '<strong class="text-emerald-500 font-black tracking-widest text-[11px] uppercase">$1</strong>');
    // Section Headers
...
function unlockExportPanel() {
    ...
    gdocBtn.classList.add('bg-slate-800', 'hover:bg-slate-700', 'text-blue-500', 'cursor-pointer');
    gdocBtn.querySelector('span').classList.replace('text-slate-600', 'text-white');
    gmailBtn.disabled = false;
    gmailBtn.classList.add('bg-slate-800', 'hover:bg-slate-700', 'text-red-500', 'cursor-pointer');
...
function lockExportPanel() {
    ...
    gdocBtn.classList.remove('bg-slate-800', 'hover:bg-slate-700', 'text-blue-500', 'cursor-pointer');
    gdocBtn.querySelector('span').classList.replace('text-white', 'text-slate-600');
    gmailBtn.disabled = true;
    gmailBtn.classList.remove('bg-slate-800', 'hover:bg-slate-700', 'text-red-500', 'cursor-pointer');
    ...
    pulseContainer.innerHTML = `
        <div id="narrative-card" class="p-10 bg-slate-900 border border-white/10 rounded-2xl min-h-[400px] flex flex-col justify-center items-center text-center">
            <div class="flex flex-col items-center justify-center animate-pulse">
                ...
                <p class="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">Restoring Intelligence Core...</p>
            </div>
        </div>
    `;
    feeCard.innerHTML = `
        <div class="flex flex-col items-center justify-center animate-pulse">
            ...
            <p class="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">Syncing Source...</p>
        </div>
    `;

    try {
        setStatus(`PULSING NETWORK: ${days}-DAY WINDOW...`);
        const scrapeData = await safeFetchJson(`${BASE_URL}/scrape?days=${days}`);

        setStatus(`HARVESTED ${scrapeData.row_count} REVIEWS. ENFORCING THEME ALIGNMENT...`);
        ...
        unlockExportPanel();
        setStatus(`PROTOCOL RESTORED. ANALYSIS READY.`);
        ...
        pulseContainer.innerHTML = `
            <div id="narrative-card" class="p-10 bg-slate-900 border border-white/10 rounded-2xl min-h-[400px] flex flex-col justify-center items-center text-center">
                <p class="text-xs font-black uppercase tracking-[0.2em] text-red-500">${err.message}</p>
            </div>
        `;
    } finally {
...
function renderPulse(report) {
    ...
    narrativeEl.className = "p-8 md:p-8 bg-slate-900 border border-white/10 rounded-3xl min-h-[450px] text-left transition-all duration-500 shadow-2xl flex flex-col";
    narrativeEl.innerHTML = `
        <div class="prose prose-sm md:prose-base prose-invert max-w-3xl mx-auto text-slate-300 text-base md:text-sm leading-relaxed font-medium">
    ...
    if (sortedThemes.length === 0) {
        ...
        emptyThemes.className = "p-10 bg-slate-900/40 border border-white/5 border-dashed rounded-2xl text-center";
        emptyThemes.innerHTML = `<p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">No cross-theme patterns detected in this dataset</p>`;
    ...
    for (const [theme, data] of sortedThemes) {
        ...
        if (domSentiment.toLowerCase() === "positive") badgeClass = "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 shadow-glow shadow-emerald-500/10";
        if (domSentiment.toLowerCase() === "negative") badgeClass = "bg-red-400/10 text-red-400 border border-red-400/20 shadow-glow shadow-red-500/10";
        ...
        card.className = 'p-6 bg-slate-900 border border-white/5 rounded-2xl hover:border-emerald-500/30 transition-all scale-animation';
        card.innerHTML = `
...
function renderFee(data) {
    const listHtml = data.bullets.map(b => {
        ...
        if (rest.length > 0) {
            return `<li class="flex items-start space-x-4"><span class="text-emerald-500 mt-1.5 flex-shrink-0">•</span><span><strong class="text-slate-100 font-black uppercase tracking-[0.2em] text-[10px] block mb-1">${header}</strong><span class="text-slate-400 text-sm">${rest.join(':')}</span></span></li>`;
        }
    ...
    feeCard.innerHTML = `
        ...
        <div class="mt-8 pt-6 border-t border-white/5 flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
            <span class="text-slate-700 italic">Checked: ${data.last_checked}</span>
            <a href="${data.source_url}" target="_blank" class="text-emerald-500 border border-emerald-500/10 px-3 py-1.5 rounded-lg bg-emerald-500/5 hover:bg-emerald-500/10 transition-colors">Source</a>
        </div>
    `;
}
...
gdocBtn.addEventListener('click', () => {
    ...
    modalMsg.textContent = "SYNCING WITH GOOGLE DOCS REPOSITORY.";
...
modalConfirm.addEventListener('click', async () => {
    ...
    try {
        ...
        const data = await safeFetchJson(`${BASE_URL}${route}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        setStatus(`Export Complete: ${data.doc_id || data.draft_id}`);
    } catch (err) { setStatus(`Export Error: ${err.message}`, true); }
});

console.log("Intelligence Interface V4.8 Restored (User Canonical Themes)");
