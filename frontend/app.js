// app.js — Phase 4.1 Layout & Workflow Refactor
...
function setStatus(msg, isError = false) {
    ...
    if (isError) {
        statusBar.className = "px-6 py-4 bg-red-400/10 border-l-4 border-red-400 text-red-400 rounded-r-xl text-sm font-medium animate-pulse";
    } else {
        statusBar.className = "px-6 py-4 bg-emerald-500/10 border-l-4 border-emerald-500 text-emerald-500 rounded-r-xl text-sm font-medium animate-pulse";
    }
}

function parseNarrativeMarkdown(text) {
    // Tighter vertical spacing for scannability
    return text.replace(/### (.*?)\n/g, '<h3 class="text-lg font-bold text-white mb-2 mt-4 first:mt-0">$1</h3>\n');
}
...
runBtn.addEventListener('click', async () => {
    ...
    narrativeCard.innerHTML = `
        <div class="flex flex-col items-center justify-center animate-pulse">
            ...
            <p class="text-slate-400 text-sm font-medium">Synthesizing Product Pulse...</p>
        </div>
    `;
    ...
    try {
        ...
        narrativeCard.innerHTML = `<p class="text-red-400 font-bold p-4 text-center text-sm">${err.message}</p>`;
    } finally {
...
function renderPulse(report) {
    // Tighter narrative layout
    narrativeCard.classList.replace('flex', 'block');
    narrativeCard.classList.remove('justify-center', 'items-center', 'text-center');
    narrativeCard.innerHTML = `<div class="prose prose-sm prose-invert max-w-none text-slate-300 leading-relaxed">${parseNarrativeMarkdown(report.narrative)}</div>`;
    ...
    for (const [theme, data] of sortedThemes) {
        ...
        const quotesHtml = data.top_quotes.slice(0, 2).map(q => 
            `<div class="text-[11px] italic text-slate-500 border-l-2 border-white/5 pl-2 mt-2 py-0.5 bg-white/5 rounded-r-md leading-snug truncate hover:whitespace-normal transition-all">"${q}"</div>`
        ).join('');

        const card = document.createElement('div');
        card.className = 'p-4 bg-slate-900/60 border border-white/10 rounded-xl hover:border-emerald-500/30 transition-all group';
        card.innerHTML = `
            <div class="flex items-center justify-between mb-2">
                <h4 class="font-bold text-slate-200 group-hover:text-emerald-500 transition-colors uppercase tracking-tight text-xs">${theme}</h4>
                <span class="px-1.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${badgeClass}">${domSentiment}</span>
            </div>
            <div class="flex items-center space-x-3 text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">
...
function renderFee(data) {
    const listHtml = data.bullets.map(b => {
        const parts = b.split(':');
        if (parts.length > 1) {
            return `<li class="flex items-start space-x-2"><span class="text-emerald-500 mt-0.5">•</span><span class="leading-snug"><strong class="text-slate-200">${parts[0]}:</strong>${parts.slice(1).join(':')}</span></li>`;
        }
        return `<li class="flex items-start space-x-2"><span class="text-emerald-500 mt-0.5">•</span><span class="leading-snug">${b}</span></li>`;
    }).join('');

    feeCard.classList.remove('flex', 'justify-center', 'items-center', 'text-center');
    feeCard.className = "p-5 bg-slate-900/40 border border-white/10 rounded-2xl min-h-[300px] text-left";
    feeCard.innerHTML = `
        <h4 class="text-emerald-500 font-black uppercase tracking-widest text-[10px] mb-3">Fee Matrix: ${data.fee_type}</h4>
        <ul class="space-y-3 text-xs text-slate-400">
            ${listHtml}
        </ul>
        <div class="mt-6 pt-4 border-t border-white/5 space-y-2">
            <div class="text-[9px] font-bold text-slate-600 uppercase tracking-wider">Sync: ${data.last_checked}</div>
            <a href="${data.source_url}" target="_blank" class="inline-flex items-center space-x-1.5 text-[11px] font-bold text-emerald-500 hover:text-emerald-400 transition-colors">
                <span>SEBI Source</span>
...
gmailBtn.addEventListener('click', () => {
    ...
    if (custom) {
        modalMsg.textContent = `Targeting Executive Distribution: ${custom}`;
        modalPasswordGroup.classList.remove('hidden');
    } else {
        modalMsg.textContent = "Initiating automated Intelligence Draft Protocol (Gmail).";
...
gdocBtn.addEventListener('click', () => {
    ...
    modalMsg.textContent = "Committing Productivity Insights to Core Documentation.";
    modalPasswordGroup.classList.add('hidden');
    approvalModal.classList.remove('hidden');
});
...
modalConfirm.addEventListener('click', async () => {
    ...
    try {
        if (currentExportAction === 'gdoc') {
            setStatus("Syncing with Global Documentation Cluster...");
            ...
            setStatus(`Transmission Handshake success (${data.doc_id})`);
            
        } else if (currentExportAction === 'gmail') {
            setStatus("Broadcasting to Intelligence Draft Subsystem...");
        ...
        console.error(err);
        setStatus(`Sync Alert: ${err.message}`, true);
    }
});
